...
The best way to learn about Kusto Query Language is to look at some basic queries to get a "feel" for the language. We recommend using a [database with some sample data](https://help.kusto.windows.net/Samples). This tutorial shows the queries that should run on that database. The `StormEvents` table in the sample database provides some information about storms that happened in the United States.
...
Count events by a one day structure, binned into hours. Here, we use `floor` instead of `bin`:
